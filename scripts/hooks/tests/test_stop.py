...
def test_verify_python_used_by_lint_and_test_checks(tmp_path, monkeypatch):
    ...
    # The script constants are resolved at import against the *real* repo root, so
    # moving REPO_ROOT does not move them. Point them at files that exist: this
    # asserts which interpreter a resolved command runs under, and `_command_for`
    # now declines to build a command at all for a script that is missing.
    present = tmp_path / "present.py"
    present.write_text("")
    monkeypatch.setattr(hook, "LINT_ALL", present)
    monkeypatch.setattr(hook, "CHECK_LOCK_MARKERS", present)
